	...
	if($single){
		if(theme_get_option('portfolio', 'featured_image_lightbox')){
			if(theme_get_option('portfolio', 'featured_image_lightbox_gallery')){
				$output .= '<a class="image_icon_zoom lightbox" href="'.$image_src_array[0].'" data-fancybox-group="post-'.get_queried_object_id().'" title="'.$title.'">';
				...
				if(!empty($attachments)){
					...
					foreach ( $attachments as $id => $attachment ) {
						...
						//$title = wptexturize( esc_html($attachment->post_excerpt) );
							$output .= '<a class="lightbox" href="'.$img_src[0].'" title="'.strip_tags(get_the_title()).'" data-fancybox-group="post-'.get_queried_object_id().'">'.$id.'</a>';
						}
					}
					$output .= '</div>';
				}
