...
function theme_section_portfolio_list($options){
	...
	while($r->have_posts()) {
		...
		if ($thumbnail_id) {
			...
			$output .= '<a class="image_icon_'.$icon.$lightbox.'" '.(isset($link_target)?'target="'.$link_target.'" ':'').' title="'. $image_title .'" href="' . $href . '"'.$rel.$width.$height.$data.$inline.$iframe.$image_lightbox_fittoview.'>';
			$output .= '<img width="'.$size[0].'" height="'.$size[1].'" src="' . $image_src . '" data-thumbnail="'.$thumbnail_id.'" title="' . get_the_title() . '" alt="' . get_the_title() . '" />';
			$output .= '</a>';
			$output .= '</div></div>';
			$output .= '</div>';
		}
		...
		if($title == 'true'){
			if($titlelinkable == 'true'){
				...
				$output .= '<div class="portfolio_title"><a href="'.$href.'"'.$target.' alt="'.get_the_title().'">' . get_the_title() . '</a></div>';
			} else{
		...
		if($type == 'gallery'&&!empty($image_ids)){
			...
			foreach($image_ids as $image_id){
				...
				$output .= '<a class="lightbox" href="'.$image_src[0].'" title="'. $image_title .'" data-fancybox-group="'.(($group=='true')?$rel_group:'gallery-'.get_the_ID()).'">gallery-'.get_the_ID().'</a>';
			}
			$output .= '</div>';
		}
		$output .= '</div>';
