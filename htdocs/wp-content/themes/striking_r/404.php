...
if(!empty($items)){
	...
	foreach($items as $item){
		switch($item){
			case 'pages':
				...
				if(!empty($content)){
					$outputs[] = '<h2>'.__('Pages', 'theme_front').'</h2>'.$content;
				}
				break;
			case 'categories':
				$exclude_cats = theme_get_option('blog','exclude_categorys');
				$content = '<h2>'.__('Category Archives', 'theme_front').'</h2>';
				$content .= '<ul>';
			...
			case 'posts':
				...
				$content = '<h2>'.__('Blog Posts', 'theme_front').'</h2>';
				$content .= '<ul>';
				while ($archive_query->have_posts()) {
					$archive_query->the_post();
					$content .= '<li><a href="'.get_permalink().'" rel="bookmark" title="'.sprintf( __("Permanent Link to %s", 'theme_front'), get_the_title() ).'">'.get_the_title().'</a> ('.get_comments_number().')</li>';
				}
				$content .= '</ul>';
				$outputs[] = $content;
				break;
	...
	$output = implode('<div class="divider top"><a href="#">'.__('Top','theme_front').'</a></div>',$outputs);
	echo $output;
}
?>
