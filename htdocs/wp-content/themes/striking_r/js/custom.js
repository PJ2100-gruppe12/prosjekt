...
jQuery(document).ready(function($) {
    ...
    if ($('body').is('.responsive')) {
        ...
        function updateImages() {
            $('.image_styled img, .product-thumbnail, .woocommerce-main-image img').each(function() {
                var $image = $(this);
    ...
    $('.responsive_text').adaptText();
    $('.milestone_number').countTo({
        speed: 1500,
        refreshInterval: 25
    });
    $('.pie_progress_wrap').each(function() {
        var $self = $(this),
            size = 150,
            lineWidth = 7,
    ...
    $('.testimonials').each(function() {
        ...
        function update(item) {
            ...
            if (item.meta) {
                if (item.link) {
                    $meta.hide().html('<a href="' + item.link + '" target="_blank">' + item.meta + '</a>').fadeIn();
                } else {
            ...
            beforeLoad: function() {
                if (this.element.is('.fancyaudio')) {
                    ...
                    this.content = '<div class="video_frame" style="width:' + this.width + 'px">' +
                        '<video id="video' + id + '" width="' + this.width + '" height="' + this.height + '" controls="controls">' +
                    ...
                    var videoplayer;
                    this.beforeShow = function() {
                        if (this.element.is('.fancymobile')) return;
                        if (MediaElementPlayer === undefined) return;
    ...
    enable_lightbox('.lightbox', document);
