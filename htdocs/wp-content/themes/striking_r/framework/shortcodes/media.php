...
_end_;
	...
	// if ($width) {
	// 	$js_options[] = 'videoWidth: ' . $width;
	// }
	// if ($height) {
	// 	$js_options[] = 'videoHeight: ' . $height;
	// }
...
HTML;
	...
	return <<<HTML
<div class="video_frame" style="width:{$width}px;height:{$height}px" data-ratio="{$ratio}">
<video id="html5_video_{$id}" {$poster_attribute} controls="controls" {$preload_attribute} {$autoplay_attribute} width="100%" height="100%">
...
function theme_video_youtube($atts, $content=null) {
	...
	$ratio = round($width/$height,2);
	if (!empty($clip_id)){
		return "<div class='video_frame' data-ratio='{$ratio}' style='height:{$height}px;width:{$width}px'><iframe class='youtube' style='height:100%;width:100%' src='http://www.youtube.com/embed/{$clip_id}?autohide={$autohide}{$autoplay}&amp;controls={$controls}&amp;disablekb={$disablekb}&amp;fs={$fs}&amp;start={$start}&amp;loop={$loop}&amp;rel={$rel}&amp;showinfo={$showinfo}&amp;theme={$theme}&amp;modestbranding={$modestbranding}&amp;wmode=transparent' width='100%' height='100%' frameborder='0' webkitAllowFullScreen mozallowfullscreen allowfullscreen></iframe></div>";
	}
}
...
//&hideInfos=0
	$ratio = round($width/$height,2);
	if (!empty($clip_id)){
		return "<div class='video_frame' data-ratio='{$ratio}' style='height:{$height}px;width:{$width}px'><iframe class='dailymotion' style='height:100%;width:100%' src='http://www.dailymotion.com/embed/video/$clip_id?width=$width&amp;autoPlay={$autoplay}&amp;related={$related}&amp;chromeless={$chromeless}&amp;theme=none&amp;foreground=%23F7FFFD&amp;highlight=%23FFC300&amp;background=%23171D1B&amp;iframe=1&amp;wmode=transparent' width='100%' height='100%' frameborder='0' webkitAllowFullScreen mozallowfullscreen allowfullscreen></iframe></div>";
	}
}
