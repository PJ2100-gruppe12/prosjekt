...
STYLES;
	...
	return '<div id="'.$blog_id.'">'.$output.'</div>'.$styles;
}
		...
		while ($r->have_posts()) : 
			...
			if($title === true || $meta === true){
				...
				if($title === true){
					...
					$output .= '<h2 class="entry_title"><a href="'.get_permalink().'" rel="bookmark" title="'.sprintf( __("Permanent Link to %s", 'theme_front'), get_the_title() ).'">'.$post_title.'</a></h2>';
				}
				if($meta === true){
