...
function theme_shortcode_masonry($atts, $content = null, $code) {
	...
	if ($r->have_posts()){
		while ($r->have_posts()){
			...
			if($title){
				$output .= '<h4 class="masonry_item_title"><a href="'.get_permalink().'" rel="bookmark" title="'.sprintf( __("Permanent Link to %s", 'theme_front'), get_the_title() ).'">'.get_the_title().'</a></h4>';
			}
			if($desc){
