...
function theme_shortcode_recent_comments($atts){
	...
	$output = '<section class="widget_recent_comments"><ul id="recentcomments">';
	if ( $comments ) {
...
function theme_shortcode_links($atts) {
	...
	if($cat_title == 'true'){
		$output = '<div class="links_wrap">'.$output.'</div>';
	}else{
		$output = '<div class="links_wrap"><ul class="xoxo blogroll">'.$output.'</ul></div>';;
	}
	return $output;
}
