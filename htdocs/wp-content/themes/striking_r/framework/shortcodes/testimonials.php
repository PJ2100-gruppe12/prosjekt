...
function theme_shortcode_testimonials($atts, $content=null){
	...
	if(isset($first['meta'])){
		if(isset($first['link'])){
			$meta_html = '<span class="testimonial_meta"><a href="'.$first['link'].'" target="_blank">'.$first['meta'].'</a></span>';
		} else {
	...
	return <<<HTML
<div class="testimonials" data-autoplay="{$autoplay}" data-duration="{$duration}" data-items="{$id}">
