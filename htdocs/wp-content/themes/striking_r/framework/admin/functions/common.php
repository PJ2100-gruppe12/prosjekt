...
function theme_update_nav_menu_item($menu_id, $menu_item_id, $args) {
    ...
    if ( isset($_POST["menu-item-icon-color"]) && is_array($_POST[ "menu-item-icon"])) {
    	foreach( $_POST["menu-item-icon-color"] as $post_id => $value ) {
			update_post_meta( $post_id, 'menu-item-icon-color', $value );
		}
    }
}
