...
function breadcrumbs_plus_get_items( $args ) {
	...
	elseif ( is_singular() ) {
		...
		if ( 'post' === $wp_query->post->post_type && $args['show_blog'] && !empty($page_for_posts)) {
			$item[] = '<a href="' . get_permalink( $page_for_posts ) . '">' . get_the_title( $page_for_posts ) . '</a>';
		}
	...
	/* If viewing any type of archive. */
		...
		if ( is_category() || is_tag() || is_tax() ) {
			...
			$item['last'] =  sprintf(__("Author Archive for: '%s'",'theme_front'),get_the_author_meta( 'display_name', $author_obj->ID ));
		}
	}
