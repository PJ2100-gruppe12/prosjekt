...
class Theme_Widget_Contact_Form extends WP_Widget {
	...
	function add_admin_script(){
		...
		wp_enqueue_script('init-widget-color',THEME_ADMIN_ASSETS_URI . '/js/init-widget-color.js',array('jquery','mColorPicker'));
	}
	...
	function widget( $args, $instance ) {
		...
		?>
		<p style="display:none;"><?php _e('Your message was successfully sent. <strong>Thank You!</strong>','theme_front');?></p>
		<form class="contact_form" action="<?php echo $action;?>" method="post" novalidate="novalidate">
			<p><input type="text" required="required" id="contact_name" name="contact_name" class="text_input" value="" size="22" tabindex="21" />
			<label for="contact_name"><?php _e('Name *', 'theme_front'); ?></label></p>
			
			<p><input type="email" required="required" id="contact_email" name="contact_email" class="text_input" value="" size="22" tabindex="22"  />
			<label for="contact_email"><?php _e('Email *', 'theme_front'); ?></label></p>
			
			<p><textarea required="required" name="contact_content" class="textarea" cols="30" rows="5" tabindex="23"></textarea></p>
			
			<p><button type="submit"<?php echo $bgColor;?> class="<?php echo apply_filters( 'theme_css_class', 'button' );?> white"><span<?php echo $textColor;?>><?php _e('Submit', 'theme_front'); ?></span></button></p>
