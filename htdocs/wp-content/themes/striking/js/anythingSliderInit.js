jQuery(document).ready(function() {
    ...
    jQuery('<div id="anything_slider_loading" class="anything_slider_loading"></div>').insertBefore($slider);
    $slider.show();
    $slider.find('img.slideimage').each(function() {
        jQuery(this).before('<div class="image-loading"/>');
        ...
        $slider.find('.anything_caption').css({
        ...
        if ($slider.find('object[data-url*=youtube], embed[src*=youtube], iframe[src*=youtube], object[data-url*=vimeo], embed[src*=vimeo], iframe[src*=vimeo], video').length > 0) {
            ...
            jQuery('.anything_slider_loading').delay(5000).animate({
                opacity: 0
            }, 2000, function() {
                jQuery(this).remove();
                base.startStop(base.options.autoPlay, true);
            });
        } else {
            jQuery('.anything_slider_loading').delay(300).animate({
                opacity: 0
            }, 2000, function() {
                jQuery(this).remove();
            });
