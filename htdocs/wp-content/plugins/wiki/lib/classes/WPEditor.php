...
final class WikiEditor {
	...
	/**
	 * This function is copied from wp-admin/includes/media.php. It will return the output instead of echo it
	 */
	public static function media_buttons($editor_id = 'content') {
		$post = get_post();
		$html = '';
		...
		return '<a href="#" id="insert-media-button" class="button insert-media add_media" data-editor="' . esc_attr( $editor_id ) . '" title="' . esc_attr__( 'Add Media' ) . '">' . $img . __( 'Add Media' ) . '</a>';
	}
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$html .= '<div id="wp-' . $editor_id . '-wrap" class="wp-core-ui wp-editor-wrap ' . $switch_class . '">';
		...
		if ( !empty($buttons) || $set['media_buttons'] ) {
			$html .= '<div id="wp-' . $editor_id . '-editor-tools" class="wp-editor-tools hide-if-no-js">';
			...
			if ( $set['media_buttons'] ) {
				...
				$html .= '<div id="wp-' . $editor_id . '-media-buttons" class="wp-media-buttons">';
				$html .= self::media_buttons($editor_id);
				$html .= "</div>\n";
			}

			$html .= '<div class="wp-editor-tabs">' . $buttons . "</div>\n";
			$html .= "</div>\n";
		}
		...
		$html .= sprintf($the_editor, $content);
		$html .= "\n</div>\n\n";
